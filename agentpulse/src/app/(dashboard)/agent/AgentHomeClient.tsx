...
const SAISIE_STAGES: {
...
}[] = [
  {
    ...
    activityType: 'CONTACT', color: 'text-blue-400 bg-blue-500/10 border-blue-500/20',
    icon: 'M2.25 6.75c0 8.284 6.716 15 15 15h2.25a2.25 2.25 0 002.25-2.25v-1.372c0-.516-.351-.966-.852-1.091l-4.423-1.106c-.44-.11-.902.055-1.173.417l-.97 1.293c-.282.376-.769.542-1.21.38a12.035 12.035 0 01-7.143-7.143c-.162-.441.004-.928.38-1.21l1.293-.97c.363-.271.527-.734.417-1.173L6.963 3.102a1.125 1.125 0 00-1.091-.852H4.5A2.25 2.25 0 002.25 4.5v2.25z',
  },
  {
    key: 'ENTRETIEN', label: 'Entretien',
    activityType: 'ENTRETIEN', color: 'text-cyan-400 bg-cyan-500/10 border-cyan-500/20',
    icon: 'M20.25 8.511c.884.284 1.5 1.128 1.5 2.097v4.286c0 1.136-.847 2.1-1.98 2.193-.34.027-.68.052-1.02.072v3.091l-3-3c-1.354 0-2.694-.055-4.02-.163a2.115 2.115 0 01-.825-.242m9.345-8.334a2.126 2.126 0 00-.476-.095 48.64 48.64 0 00-8.048 0c-1.131.094-1.976 1.057-1.976 2.192v4.286c0 .837.46 1.58 1.155 1.951m9.345-8.334V6.637c0-1.621-1.152-3.026-2.76-3.235A48.455 48.455 0 0011.25 3c-2.115 0-4.198.137-6.24.402-1.608.209-2.76 1.614-2.76 3.235v6.226c0 1.621 1.152 3.026 2.76 3.235.577.075 1.157.14 1.74.194V21l4.155-4.155',
  },
  {
    key: 'ENTREVUE', label: 'Entrevue',
    activityType: 'ENTREVUE', color: 'text-violet-400 bg-violet-500/10 border-violet-500/20',
    icon: 'M9 12h3.75M9 15h3.75M9 18h3.75m3 .75H18a2.25 2.25 0 002.25-2.25V6.108c0-1.135-.845-2.098-1.976-2.192a48.424 48.424 0 00-1.123-.08m-5.801 0c-.065.21-.1.433-.1.664 0 .414.336.75.75.75h4.5a.75.75 0 00.75-.75 2.25 2.25 0 00-.1-.664m-5.8 0A2.251 2.251 0 0113.5 2.25H15c1.012 0 1.867.668 2.15 1.586m-5.8 0c-.376.023-.75.05-1.124.08C9.095 4.01 8.25 4.973 8.25 6.108V8.25m0 0H4.875c-.621 0-1.125.504-1.125 1.125v11.25c0 .621.504 1.125 1.125 1.125h9.75c.621 0 1.125-.504 1.125-1.125V9.375c0-.621-.504-1.125-1.125-1.125H8.25zM6.75 12h.008v.008H6.75V12zm0 3h.008v.008H6.75V15zm0 3h.008v.008H6.75V18z',
  },
  {
    key: 'SOUMISE', label: 'Police soumise',
    activityType: 'NOTE', note: 'Soumission de police',
    color: 'text-amber-400 bg-amber-500/10 border-amber-500/20',
    icon: 'M19.5 14.25v-2.625a3.375 3.375 0 00-3.375-3.375h-1.5A1.125 1.125 0 0113.5 7.125v-1.5a3.375 3.375 0 00-3.375-3.375H8.25m0 12.75h7.5m-7.5 3H12M10.5 2.25H5.625c-.621 0-1.125.504-1.125 1.125v17.25c0 .621.504 1.125 1.125 1.125h12.75c.621 0 1.125-.504 1.125-1.125V11.25a9 9 0 00-9-9z',
  },
  {
    key: 'EMISE', label: 'Police émise',
    activityType: 'NOTE', note: 'Police émise',
    color: 'text-emerald-400 bg-emerald-500/10 border-emerald-500/20',
    icon: 'M9 12.75L11.25 15 15 9.75M21 12a9 9 0 11-18 0 9 9 0 0118 0z',
  },
  {
    key: 'LIVRAISON', label: 'Livraison',
    activityType: 'NOTE', note: 'Livraison effectuée',
...
// ─── Composant principal ──────────────────────────────────────────────────────
export default function AgentHomeClient({ stats }: { stats: Stats }) {
  const { banner, pipeline, ratios } = stats

  // Compteurs locaux pour la saisie rapide (feedback immédiat)
  ...
  return (
    <div className="flex flex-col gap-6">
      ...
      <div className="relative rounded-2xl overflow-hidden bg-gradient-to-br from-[#0D1626] via-blue-950/60 to-[#0D1626] border border-blue-500/20 p-6">
        {/* Fond décoratif */}
        <div className="absolute inset-0 bg-[radial-gradient(ellipse_at_top_right,rgba(59,130,246,0.12),transparent_60%)] pointer-events-none" />
        ...
        <div className="grid grid-cols-3 gap-4 mb-6">
          <BannerKpi
            ...
            value={String(banner.policesEmisesMois)}
            sub={`objectif : ${banner.objectifMensuel}`}
            color="text-blue-300"
          />
          <BannerKpi
            label="Prime moyenne"
            value={formatFcfa(banner.primeMoyenne)}
            color="text-amber-300"
          />
          <BannerKpi
            label="Taux global"
            value={pct(banner.tauxGlobal)}
      ...
      {/* ── SAISIE RAPIDE + RATIOS (côte à côte sur grand écran) ── */}
      <div className="grid grid-cols-1 lg:grid-cols-2 gap-6">
        ...
        <div className="rounded-2xl border border-white/10 bg-white/[0.03] p-5">
          ...
          <div className="flex flex-col gap-2">
            {SAISIE_STAGES.map(stage => {
              ...
              const todayHit = quickCounts[stage.key]
              const isFlash  = flashing === stage.key

              return (
                <div
                  key={stage.key}
                  className={`flex items-center gap-3 rounded-xl border px-4 py-3 transition-colors ${stage.color}`}
                >
                  {/* Icône */}
        ...
        <div className="rounded-2xl border border-white/10 bg-white/[0.03] p-5">
          ...
          <div className="grid grid-cols-2 gap-3">
            <RatioCard
              label="Taux global"
              value={pct(ratios.tauxGlobal)}
              hint="Livraisons / total"
              color="text-blue-400"
            />
            <RatioCard
              label="Prime journalière"
              value={formatFcfa(ratios.primeJournaliere)}
              hint="Livrées aujourd'hui"
              color="text-amber-400"
            />
            <RatioCard
              label="Soumises → émises"
              value={pct(ratios.soumisesEmises)}
              hint="Taux de transformation"
              color="text-emerald-400"
            />
            <RatioCard
              label="Taux terrain"
              value={pct(ratios.tauxTerrain)}
              hint="Entretiens / contacts"
              color="text-cyan-400"
            />
            <RatioCard
              label="Taux qualification"
              value={pct(ratios.tauxQualification)}
              hint="Entrevues / entretiens"
              color="text-violet-400"
            />
            <RatioCard
              label="Livraisons"
              value={String(ratios.livraisons)}
              hint="Total pipeline"
...
function RatioCard({
  ...
  return (
    <div className="rounded-xl border border-white/8 bg-white/[0.03] p-4">
      <p className="text-[11px] text-white/40 leading-tight mb-1">{label}</p>
